...
def call_model_node(state: AgentState) -> Dict[str, Any]:
	...
	# Get the LLM that knows about our tools
	...
	# Build prompt and add conversation history
	...
	try:
		...
		# Prepare State updates with the LLM's reply
		...
		# Check if the LLM wants to use a tool, and track what it is trying to do
		if hasattr(response, "tool_calls") and response.tool_calls:
			...
			# Map the specific tool to our standard intents
...
def execute_tool_node(state: AgentState) -> Dict[str, Any]:
	...
	# Grab the last message from the agent
	...
	# If the LLM didn't suggest any tools, we skip and return nothing
	if not hasattr(last_message, "tool_calls") or not last_message.tool_calls:
		return {}

	# Map LLM tool names to our actual Python functions
	tools_map = {
	...
	# To keep the API simple, we only save the first tool's result to the state.
	...
	for tool_call in last_message.tool_calls:
		...
		if tool_func:
			try:
				# Actually run the python function mapped to the tool
				result = tool_func.invoke(tool_args)
				
				# Save the first one we find so our API endpoint can see it
				if primary_tool_result is None:
					primary_tool_result = result
				
				# Log the tool output as a ToolMessage to feed back to the LLM
				new_messages.append(
					ToolMessage(
						content=str(result),
				...
				# Catch errors, save them as ToolMessages so the LLM knows it failed
				new_messages.append(
					ToolMessage(
	...
	# If any tool executed was 'escalate', flag this conversation
	if any(tc["name"] == "escalate" for tc in last_message.tool_calls):
		updates["escalate"] = True

	# Add the raw result string we intercepted
...
def format_response_node(state: AgentState) -> Dict[str, Any]:
	...
	# We look at the most recent message
	last_message = messages[-1]

	# We only format text if the last thing that ran was a tool (DB query, etc.)
	if not isinstance(last_message, ToolMessage):
		return {}

	# Grab the LLM that DOES NOT have access to tools
	llm = _get_plain_llm()
	
	# If the LLM failed to load, fallback to dumping the raw database text
	if llm is None:
		return {"final_response": str(last_message.content)}

	# Wrap the tool output in the system prompt so it speaks nicely to the user
	all_messages = [SystemMessage(content=_build_system_prompt())] + messages

	try:
		# Ask the plain LLM to summarize the tool info
		...
		# Return the final summary text the human user will actually see
		return {
			"messages": [response],
